...
<div>
   @if ($issueRequests)
   <!-- if the user has recent issues -->
   <div class="flex flex-row antialiased text-gray-800 " style="height: 90vh;">
       <div class="flex flex-row flex-shrink-0 p-4 bg-gray-100 w-96 {{ $showChats ? 'lg:block hidden' : '' }}">
               ...
               @if (count($waitingRequests) > 0)
                   <div class="flex flex-row mb-3">
                       <button wire:click="$toggle('waitingList')"
                           class="flex px-4 py-1 text-white uppercase bg-purple-500 rounded hover:bg-purple-400">
                           Support Queue
                       </button>
                   </div>
               @endif
               <div class="flex flex-row items-center">
                   ...
                   <div class="ml-auto">
                       <button wire:click="$toggle('newRequest')"
                           ...
                           <?php echo '<?xml version="1.0" encoding="iso-8859-1" ?>' ?>                           <!-- Generator: Adobe Illustrator 19.0.0, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
               ...
               <div class="overflow-y-auto">
                   <div class="mt-5">
                       <div class="flex flex-col -ml-4">
                           @foreach ($issueRequests as $issue)
                               <a href="#" wire:click="selectedChat({{$issue->id}})" class="cursor-pointer">
                                   <div
                                       ...
                                       <div class="flex items-center justify-center flex-shrink-0 w-10 h-10">
                                           <img src="{{ $issue->servitor->profile_photo_url }}"
                                               alt="{{ $issue->servitor->name }}"
                                               class="w-full h-full font-bold rounded-full">
                                       </div>
                                       <div class="flex flex-col flex-grow ml-3 mr-12">
                                           <div class="text-sm font-medium">
                                               {{ ucfirst($issue->servitor->name) }}</div>
                                           <div class="w-40 text-xs truncate">
                                               @if ($issue->sender_id == Auth::user()->id)
                                                   Me:
                                               @endif
                                               {{ $issue->chats->last()->context }}
                                           </div>
                                       </div>
                                       @if (!$issue->read && $issue->sender_id != Auth::user()->id)
                                           <div class="self-end flex-shrink-0 mb-1 ml-2">
                                               <span
                                                   class="flex items-center justify-center text-xs text-white bg-blue-500 rounded-full"
                                                   style="height: 0.5rem; width:0.5rem;"></span>
                                           </div>
                                       @endif
                                   </div>
                               </a>
       ...
       {{-- if true show messages component --}}
       @if ($showChats)
    ...
    <div class="relative flex flex-col w-full h-full px-4 py-6 bg-white ">
        <p class="py-4 text-base font-bold text-green-500 md:text-sm lg:hidden">&lt;&lt;
            <a href="#" wire:click="$toggle('showChats')"
        ...
        <div class="h-full pt-4 overflow-hidden">
            <div class="h-full overflow-y-auto" id="messages">
         ...
         <!-- previous-->
            ...
            @if (!$request->resolved)
            <div class="absolute bottom-0 flex flex-row items-center w-full my-4 ">
    ...
    @else
    <!-- new issue mount -->
    @livewire('app.support.new-issue')
   @endif
   <!-- modals -->
   ...
   <!--new request modal-->
        <x-jet-dialog-modal wire:model="newRequest">
            ...
            <x-slot name="content">
                ...
                field
                below and we will quickly get back to you in 5 - 10 minutes during office times.
