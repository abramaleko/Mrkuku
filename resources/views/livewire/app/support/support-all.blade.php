                    ...
                    <div class="mt-5">
                        <div class="flex flex-col -ml-4">
                            @foreach ($investorRequests as $issue)
                                <a href="#" wire:click="$emitUp('selectMessage',{{ $issue->support_id }})" class="cursor-pointer">
                                    <div
                                        ...
                                        <div class="flex items-center justify-center flex-shrink-0 w-10 h-10">
                                            <img src="{{ $issue->request->servitor->profile_photo_url }}"
                                                alt="{{ $issue->request->servitor->name }}"
                                                class="w-full h-full font-bold rounded-full">
                                        </div>
                                        <div class="flex flex-col flex-grow ml-3 mr-12">
                                            <div class="text-sm font-medium">
                                                {{ ucfirst($issue->request->servitor->name) }}</div>
